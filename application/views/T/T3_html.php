...
<div class="left">
  ...
  <!----------------------------------------------------experience----------------------------------------------------------------------------> 
      ...
      <div class="experience">
      	<h1>Experience Summary</h1>
				<div class="exp">   
          <div class="exp_yers">
          	<p class="exp_yers_btm">
            <span>Total: <?=$user_detail['experience'];?> &nbsp;&nbsp;&nbsp;&nbsp;</span>
            </p><div class="clearall"></div>
          </div>
          <div class="ctc">
            <h2 class="cctc">Current CTC: <span><?=$compensation[0];?>/-</span></h2>
            <h3 class="ectc">Expected CTC: <span><?=$compensation[1];?>/-</span></h3>
          </div>
          <div class="spr">&nbsp;</div>
          <div class="clearall"></div>     	
				 <? 
          ...
          ?>
        	<p class="desigination"><?=$cmpnyDesg[$i];?></p>
          <p class="company"><?=$cmpny;?></p><div class="clearall"></div>
          <p class="from"><?=$cpmpanyData[0];?>&nbsp;&nbsp;&nbsp;<?=$cpmpanyData[1];?></p>
          <div class="clearall"></div>
					<? $i++; } ?>
        </div>
 ...
 <!----------------------------------------------------skills----------------------------------------------------------------------------> 
      ...
      <div class="skills">
          ...
          ?>
        	<p class="desigination"><?=$name;?></p>
          <p class="level"><?=$skillEff[$i];?></p>
          <? $i++; } ?>
          <p class="desc">&nbsp;</p>
        </div>
		</div><div class="clearall"></div>
 ...
 <!----------------------------------------------------project----------------------------------------------------------------------------> 
      ...
      <div class="project">
        ...
        ?>
        	<p class="project_name"><?=$proj;?></p>
          <p class="role"><?=$projRole[$i];?></p><div class="clearall"></div>
          <p class="url"><?=$projUrl[$i];?></p>
          <div class="clearall"></div>
          <p class="descp_tit">Project Description</p>
          <p class="descp"><?=$projDesc[$i];?></p>
   ...
   <!-- --------------------------------------------------awards-------------------------------------------------------------------------- --> 
      ...
      <div class="awards">
          ...
          <? 
					...
					?>
        	<p class="award_name"><?=$awd;?></p>
          <p class="yers"><?=$awardDate[0];?> &nbsp;&nbsp;&nbsp; <?=$awardDate[1];?></p><div class="clearall"></div>
          
          <div class="clearall"></div>
          <p class="descp"><?=$awdDesc[$i];?></p>
          <? $i++; } ?>
      </div>
		</div><div class="clearall"></div>
    <? } ?> 
      ...
      <div class="interest">
      	<h1>My Other Interest</h1>
            <? 
							...
							?>
				<div class="inte">   	
        	<p class="inte_name"><?=$title;?></p>
          <p class="url"><?=$intrestUrl[$i];?></p>
          <div class="clearall"></div>
          <p class="descp"><?=$intrestDesc[$i];?></p>
       </div>
       <? $i++; } ?>
		</div><div class="clearall"></div> 
    <? } ?> 
...
<!---------------------------------------------------contact----------------------------------------------------------------------------->   
 <div class="contact">
  	<h1>Contact Me</h1>
    <p class="phone"><?=$user_detail['mobile'];?></p>
    <p class="mail"><?=$user_detail['secondary_email'];?></p>
    <p class="address"><?=nl2br($user_detail['address']);?></p>
  </div><div class="clearall"></div>
...
<!---------------------------------------------------education----------------------------------------------------------------------------->  
  ...
  <div class="education">
    ...
    <? 
							...
							?>
     <p class="certification_name"><?=$eduCert[$i];?></p>
     <p class="institution"><?=$edu;?></p><div class="clearall"></div>
     <p class="from"><?=$educationDate[0];?>&nbsp;&nbsp;&nbsp;<?=$educationDate[1];?></p>
     <p class="percentage"><?=$eduScore[$i];?></p>
     <? $i++; } ?>
     </div>
  </div><div class="clearall"></div>
  <? } ?>
  ...
  <div class="other_details">
    ...
    <div class="sub">
      <? if($user_detail['married']!=NULL)
      {
		  ...
		  $married = 'Unmarried';
		  ?>
     <p class="mstatus"><b>Marital status:</b> <?=$married?></p>
     <? }?>
     <? if($passport_visa['passport'])
      {
		  //$passportDate=explode('#',$passport_visa['passportdate']);
		  ?>
     <p class="passport"><b>Passport details:</b> <?=$passport_visa['passport'];?>, Valid: <?php //$passportDate[0]; ?>till <?=$passport_visa['passportTo'];?></p>
     <? }?>
      <? if($passport_visa['visa'])
      {
		  //$visaDate= explode('#',$passport_visa['visadate']);
		  ?>
     <p class="visa"><b>Visa details:</b>  <?=$passport_visa['visa'];?>, Valid: <?php //$visaDate[0];?>till <?=$passport_visa['visaTo'];?></p>
     <? }?>
     <p class="lin"><?=$website['mylink'];?></p>
     <? if($user_detail['skype']) { ?><p class="skype"><?=$user_detail['skype'];?></p><? } ?>
     <? if($website['linkedin']) { ?><p class="fb"><?=$website['linkedin'];?></p><? } ?>
     <? if($website['twitter']) { ?><p class="lin"><?=$website['twitter'];?></p><? } ?>
     <? if($website['facebook']) { ?><p class="twit"><?=$website['facebook'];?></p><? } ?>
